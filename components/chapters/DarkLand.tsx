      ...
      <div className="space-y-8">
        {battles.map((quest, i) => (
          <motion.button
            ...
            className="group relative block w-full overflow-hidden rounded-2xl border border-orange-500/30 bg-gradient-to-r from-black/40 to-red-950/30 p-8 text-left backdrop-blur-sm transition hover:border-orange-400/60"
            style={{ boxShadow: "0 0 40px -20px rgba(255,140,66,0.4)" }}
          >
            <p className="mb-2 text-xs font-semibold uppercase tracking-[0.3em] text-orange-400">
              Boss Battle · {quest.year}
            </p>
            <h3 className="mb-3 text-2xl font-bold text-white sm:text-3xl">{quest.title}</h3>
            <p className="mb-5 max-w-2xl text-white/75">{quest.tagline}</p>
